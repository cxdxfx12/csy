 ...
 * 经理端全局搜索 - 大圣导航助手
 */
class Search extends BaseManager
{
    /**
     * 全局搜索
     */
    public function global()
    {
        ...
        // 经理端的小区权限过滤
        $roleId = $this->managerInfo['role_id'] ?? 0;
        $managedIds = $this->request->managedCommunityIds ?? [];
        // role_id 1=超管 2=系统管理员 → 不过滤；3=项目经理 → 过滤
        $filter = ($roleId <= 2) ? null : $managedIds;

        // ========== 搜索各项 ==========
        $this->searchOwner($keyword, $filter, $results);
        $this->searchRoom($keyword, $filter, $results);
        $this->searchBuilding($keyword, $filter, $results);
        $this->searchCommunity($keyword, $results);
        $this->searchRepairOrder($keyword, $filter, $results);
        $this->searchBill($keyword, $filter, $results);
        $this->searchVehicle($keyword, $filter, $results);
        $this->searchStaff($keyword, $results);
        $this->searchNotice($keyword, $filter, $results);
        $this->searchSupplier($keyword, $results);
        $this->searchPayment($keyword, $filter, $results);
        $this->searchAccessCard($keyword, $filter, $results);
    ...
    /**
     * 社区过滤辅助：null=不过滤，array=按ID列表过滤
     */
    private function applyFilter($query, $communityField, $filter)
    {
        if ($filter === null) return;
        if (!empty($filter)) {
            $query->where($communityField, 'in', $filter);
        }
    }

    // ========== 业主搜索 ==========
    private function searchOwner($keyword, $filter, &$results)
    {
        $q = Db::name('owner')->alias('o')
            ->leftJoin('community c', 'c.id = o.community_id')
            ->field('o.id, o.realname, o.phone, o.id_card, o.status, c.name as community_name')
            ->whereNull('o.delete_time')
            ->where('o.realname|o.phone|o.id_card', 'like', "%{$keyword}%");
        $this->applyFilter($q, 'o.community_id', $filter);
    ...
    // ========== 房间搜索 ==========
    private function searchRoom($keyword, $filter, &$results)
    {
        ...
        $this->applyFilter($q, 'r.community_id', $filter);
    ...
    // ========== 楼栋搜索 ==========
    private function searchBuilding($keyword, $filter, &$results)
    {
        ...
        $this->applyFilter($q, 'b.community_id', $filter);
    ...
    // ========== 报修工单搜索 ==========
    private function searchRepairOrder($keyword, $filter, &$results)
    {
        ...
        $this->applyFilter($q, 'ro.community_id', $filter);
    ...
    // ========== 账单搜索 ==========
    private function searchBill($keyword, $filter, &$results)
    {
    ...
    // ========== 缴费记录搜索 ==========
    private function searchPayment($keyword, $filter, &$results)
    {
        ...
        $this->applyFilter($q, 'p.community_id', $filter);
    ...
    // ========== 车辆搜索 ==========
    private function searchVehicle($keyword, $filter, &$results)
    {
        ...
        $this->applyFilter($q, 'v.community_id', $filter);
    ...
    // ========== 公告搜索 ==========
    private function searchNotice($keyword, $filter, &$results)
    {
        ...
        $this->applyFilter($q, 'n.community_id', $filter);
    ...
    // ========== 门禁卡搜索 ==========
    private function searchAccessCard($keyword, $filter, &$results)
    {
        ...
        $this->applyFilter($q, 'ac.community_id', $filter);
