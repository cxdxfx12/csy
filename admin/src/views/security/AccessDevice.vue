<template>
  <div class="page-container">
    ...
    <div class="stats-row">
      ...
      <div class="stat-item in"><span class="stat-num">{{ stats.pass_count || 0 }}</span><span class="stat-label">放行</span></div>
      <div class="stat-item out"><span class="stat-num">{{ stats.deny_count || 0 }}</span><span class="stat-label">拒绝</span></div>
    ...
    <el-card shadow="never" class="table-card">
      <el-table :data="list" v-loading="loading" stripe border @row-click="showEvents">
        ...
        <el-table-column prop="door_name" label="门点" width="130" />
    ...
    <!-- 通行事件弹窗 -->
    <el-dialog v-model="eventVisible" title="通行事件（最近50条）" width="900px" destroy-on-close>
      <el-table :data="events" stripe border max-height="400">
        <el-table-column type="index" label="#" width="50" />
        <el-table-column prop="card_no" label="卡号" width="130" />
        <el-table-column prop="holder_name" label="持卡人" width="90" />
        <el-table-column prop="door_name" label="门点" width="100" />
        <el-table-column label="方向" width="70">
          <template #default="{r}"><el-tag :type="r.direction==='in'?'success':'warning'" size="small">{{ r.direction==='in'?'进门':'出门' }}</el-tag></template>
        </el-table-column>
        <el-table-column label="开门方式" width="90">
          <template #default="{r}"><el-tag effect="plain" size="small">{{ methodMap[r.open_method] || r.open_method }}</el-tag></template>
        </el-table-column>
        <el-table-column label="动作" width="80">
          <template #default="{r}"><el-tag :type="r.action==='pass'?'success':'danger'" size="small">{{ r.action==='pass'?'放行':'拒绝' }}</el-tag></template>
        </el-table-column>
    ...
    <el-dialog v-model="formVisible" :title="formTitle" width="500px" destroy-on-close>
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="门点名称">
          <el-input v-model="form.door_name" placeholder="如：南门" />
        </el-form-item>
        <el-form-item label="设备名称" required>
          <el-input v-model="form.device_name" placeholder="如：南门控制器" />
        </el-form-item>
        <el-form-item label="品牌">
...
const methodMap: Record<string,string> = {
  card: '刷卡', remote: '远程开门', password: '密码', fingerprint: '指纹', face: '人脸',
}

const form = reactive({
  community_id:undefined as any, door_name:'', device_name:'', brand:'generic',
  device_sn:'', ip_address:'', port:80, remark:'',
})
const query = reactive({
  community_id:undefined as any, brand:'', online:'', keyword:'', page:1, limit:15,
})
...
async function loadData() {
  loading.value = true
  try {
    const r = await apiGet('/admin/security/accessDeviceList', { ...query })
...
async function loadStats() {
  try {
    const r = await apiGet('/admin/security/accessDeviceTodayStats', { community_id: query.community_id || 0 })
...
function openAdd() {
  ...
  Object.assign(form, { community_id:undefined, door_name:'', device_name:'', brand:'generic', device_sn:'', ip_address:'', port:80, remark:'' })
  formTitle.value = '添加设备'; formVisible.value = true
}

function openEdit(row: any) {
  isEdit.value = true; editId.value = row.id
  Object.assign(form, { community_id:row.community_id, door_name:row.door_name||'', device_name:row.device_name, brand:row.brand||'generic', device_sn:row.device_sn||'', ip_address:row.ip_address||'', port:row.port||80, remark:row.remark||'' })
...
async function submitForm() {
  ...
  try {
    const url = isEdit.value ? '/admin/security/accessDeviceEdit' : '/admin/security/accessDeviceAdd'
...
async function showEvents(row: any) {
  ...
  try {
    const r = await apiGet('/admin/security/accessDeviceEvents', { device_id: row.id, community_id: row.community_id })
...
async function handleDelete(row: any) {
  try {
    ...
    await apiPost('/admin/security/accessDeviceDelete', { id: row.id })
...
.stat-item.out { border-left:3px solid #f56c6c; }
