...
class Arrears extends BaseAdmin
{
    ...
    public function lists()
    {
        ...
        if ($cid === -1) {
            ...
            $placeholders = implode(',', array_fill(0, count($boundIds), '?'));
            $billSubWhere .= " AND b.community_id IN ({$placeholders})";
        } elseif ($cid > 0) {
        ...
        if ($cid === -1) {
            $boundIds = $this->request->boundCommunityIds;
            $placeholders = implode(',', array_fill(0, count($boundIds), '?'));
            $sql .= " AND o.community_id IN ({$placeholders})";
            // 子查询中已使用了同数量的占位符，需在参数数组中重复一次
            $params = array_merge($boundIds, $boundIds);
        } elseif ($cid > 0) {
